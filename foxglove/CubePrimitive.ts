...
function createBaseCubePrimitive(): CubePrimitive {
  return { pose: undefined, size: undefined, color: undefined };
}

export const CubePrimitive: MessageFns<CubePrimitive> = {
  encode(message: CubePrimitive, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): CubePrimitive {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseCubePrimitive();
  ...
  fromJSON(object: any): CubePrimitive {
    return {
  ...
  toJSON(message: CubePrimitive): unknown {
    const obj: any = {};
    if (message.pose !== undefined) {
  ...
  create<I extends Exact<DeepPartial<CubePrimitive>, I>>(base?: I): CubePrimitive {
    return CubePrimitive.fromPartial(base ?? ({} as any));
  },
  fromPartial<I extends Exact<DeepPartial<CubePrimitive>, I>>(object: I): CubePrimitive {
    const message = createBaseCubePrimitive();
